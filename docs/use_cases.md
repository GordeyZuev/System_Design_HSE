...
## Борьба с амплификацией нагрузки на БД при падении кэша

**Способы защиты в нашем проекте:**

1. **Read Replicas**
   - Чтение офферов и аренд идет на реплики
   - При падении кэша нагрузка распределяется между репликами
   - Master защищен от read-амплификации

2. **Rate Limiting на уровне сервиса**
   - Ограничение RPS на создание офферов per user
   - Защита от массовых запросов при падении кэша
   - Backpressure для клиентов

3. Холодный старт
   - При рестарте сервиса не все запросы идут в БД сразу
   - Постепенное заполнение кэша по мере поступления запросов
   - Можно добавить pre-warming популярных тарифов при старте

4. **Мониторинг и алерты**
   - Prometheus метрики для cache hit/miss ratio
   - Алерты при падении hit rate ниже порога (например, < 80%)
   - Автоматическое масштабирование БД реплик при росте нагрузки
