...
**Исходные данные:**
- X = 1000 RPS (пиковая нагрузка - чтение статуса/стоимости активных аренд)
- Y = 100 RPS (запись - старт/завершение аренды)
- Z = 100 КБ (размер одной записи аренды с событиями и индексами)
- Соотношение read:write = 10:1

**Целевая нагрузка:** 1M пользователей, 2 аренды/месяц/пользователь

**Rental Command Service (3 года retention):**
- Аренд за 3 года: 1M * 2 * 12 * 3 = **72M записей**
- Объем: 72M * 100 КБ = **7.2 TB**
- **Нагрузка:** 100 RPS write, 1000 RPS read

**Offer Service (7 дней retention):**
- ~500K офферов активных (2M/месяц * 7/30 дней)
- Размер оффера ~5 КБ (меньше аренды)
- Объем: 500K * 5 КБ = **2.5 GB**
- **Нагрузка:** ~100 RPS write (создание офферов), ~200 RPS read (валидация)
...
**Почему `user_id`:**
- Все запросы содержат `user_id` → single-shard query
- Данные пользователя изолированы
- Офферы и аренды одного user на одном шарде → data locality

**Текущая конфигурация (MVP):**
- **Rental Service:** 2 шарда 
- **Offer Service:** 2 шарда
- Каждый шард = PostgreSQL инстанс в Docker
- Роутинг на уровне приложения

#### План роста

| Пользователи | Шардов | TB/шард (Rental) | RPS/шард (write/read) |
|--------------|--------|------------------|-----------------------|
| 100K (MVP) | **2** | 0.36 TB | 50w / 500r |
| 500K | **4** | 0.9 TB | 25w / 250r |
| 1M | **8** | 0.9 TB | 13w / 125r |
| 5M | **16** | 2.25 TB | 6w / 63r |

**Критерии добавления шарда:** Storage > 200GB или CPU > 70% или p99 > 200ms

**Процесс добавления:**
1. Создать новые PostgreSQL инстансы
2. Downtime на rehashing данных (~30-60 мин для 1M users)
3. Обновить конфигурацию, рестарт
