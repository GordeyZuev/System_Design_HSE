...
- **User Service (некритичная):** Timeout 2s, без retry. Fallback — greedy pricing. Клиент получает оффер с максимальной ставкой.
- **Tariff Service (критичная):** Timeout 3s, без retry. LRU-кэш (TTL 10 мин, 1000 записей). При cache MISS + ошибка — HTTP 503, оффер не создаётся.
- **Config Service (некритичная):** Timeout 1s, TTL-кэш 60s. При отказе используется закэшированная конфигурация или дефолты (offer_ttl=300s).
- **БД:** При ошибке записи транзакция откатывается, HTTP 500.
...
**Поведение при отказе зависимостей:**
- **Offer & Pricing Service (критичная):** При недоступности валидации оффера — HTTP 503, аренда не создаётся. Все операции в транзакции, при ошибке — полный откат.
- **Stations Adapter (критичная):** При недоступности резервирования/возврата — HTTP 503, операция откатывается. Аренда остаётся в исходном состоянии.
- **Payments (некритичная):** Запись в outbox_payments создаётся в той же транзакции. Обработка асинхронная, при ошибке записи в outbox транзакция откатывается.
- **БД:** При ошибке чтения/записи транзакция откатывается, HTTP 500.
...
- Примечание: SELECT FOR UPDATE не реализован, возможны race conditions при одновременных запросах
