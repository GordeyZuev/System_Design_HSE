...
class RentalRepository:
    ...
    async def get_by_offer_id_active(self, offer_id: UUID):
        q = select(models.Rental).where(
            models.Rental.offer_id == offer_id,
    ...
    async def finish_rental(
        self, rental_id: UUID, finish_time: datetime, final_cost: Decimal
    ):
        ...
        self.session.add(r)
        snap = models.RentalCostSnapshot(
            rental_id=r.rental_id, cost_amount=final_cost, details={}
        )
    ...
    async def insert_event(
        self, rental_id: UUID, event_type: str, payload: dict
    ):

        ev = models.RentalEvent(
            rental_id=rental_id, type=event_type, payload=payload
        )
        self.session.add(ev)
        await self.session.flush()
        return ev
    ...
    async def get_rental_by_id(self, rental_id: UUID):
        result = await self.session.execute(
            select(models.Rental).where(models.Rental.rental_id == rental_id)
        )
